            ...
            <tr>
                <th v-for="(column, index) in columns" :class="alignCls(column)">
                    <div :class="cellClasses(column)">
                        <template v-if="column.type === 'selection'"><i-checkbox :value="isSelectAll" @on-change="selectAll"></i-checkbox></template>
                        <template v-else-if="column.type === 'operation'"><i-icon type="levels" size="16" style="transform: rotate(90deg)"></i-icon></template>
                        <template v-else>
                            <span v-html="renderHeader(column, index)"></span>
                            <span :class="[prefixCls + '-sort']" v-if="column.sortable">
                                <i class="fv-icon fv-icon-arrow-up-b" :class="{on: column._sortType === 'asc'}" @click="handleSort(index, 'asc')"></i>
                                <i class="fv-icon fv-icon-arrow-down-b" :class="{on: column._sortType === 'desc'}" @click="handleSort(index, 'desc')"></i>
                            </span>
                            <i-poptip
                                ...
                                <span :class="[prefixCls + '-filter']">
                                    <i class="fv-icon fv-icon-funnel" :class="{on: column._isFiltered}"></i>
                                </span>
...
<script>
    import iCheckboxGroup from '../checkbox/checkbox-group.vue';
    import iCheckbox from '../checkbox/checkbox.vue';
    import iPoptip from '../poptip/poptip.vue';
    ...
    export default {
        name: 'iTableHead',
        mixins: [ Mixin, Locale ],
        components: { iCheckboxGroup, iCheckbox, iPoptip, iButton },
        props: {
            prefixCls: String,
            styleObject: Object,
            columns: Array,
